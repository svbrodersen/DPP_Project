...
def dummy_data (n: i64) (m: i64) (overlap_pct: f64) : ([n]aabb, [m]point) =
  ...
  -- Generate rectangles with controlled overlap
  let recs =
      ...
      let base_size = 2.0  -- Rectangle width/height
      ...
      let region_scale = f64.max 0.3 (1.0 - overlap_pct * 0.7)
      let region_size = f64.sqrt (f64.i64 n) * base_size * region_scale
  ...
  let max_coord = if n == 0 then 10.0 
                  else
                    let region_scale = f64.max 0.3 (1.0 - overlap_pct * 0.7)
                    let region_size = f64.sqrt (f64.i64 n) * 2.0 * region_scale
                    in region_size + 2.0
  
  let random_values = map (\e ->
